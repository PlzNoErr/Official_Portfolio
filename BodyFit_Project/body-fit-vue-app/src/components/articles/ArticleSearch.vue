<template>
  <div class="container">
    <h3 class="text-center" style="font-family: 'Gowun Dodum', sans-serif;">자유 게시판</h3>
    <hr />
    ...
    <table class="table" style="text-align: center;">
      ...
      <tr v-for="(article, index) in articles.searchedArticleList" :key="index" style="font-family: 'Gowun Dodum', sans-serif;">
        ...
        <td>
          <a
            class="a-for-follow"
            @click="clickedFollow(article.userSeq, article.nickName)"
  ...
  methods: {
    ...
    articleCreate() {
      if (!this.loginUser.userId) {
        if (
          confirm(
            "로그인이 필요한 서비스 입니다! 로그인 창으로 이동하시겠습니까?"
          )
        ) {
